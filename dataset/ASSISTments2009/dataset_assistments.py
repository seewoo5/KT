...
# from dataset.ASSISTments2009.constant import *
from constant import *

class ASSISTDataset(Dataset):

    def __init__(self, name, sample_infos):
    ...
    def get_sequence(self, sample):
        ...
        for i in range(len(input_list)):
            if is_correct_list[i] == 0:
                input_list[i] += question_num['ASSISTments2009']
        # TODO: separate this logic as a function
        if len(input_list) > args.seq_size:
            input_list = input_list[-args.seq_size:]
        else:
            pad_counts = args.seq_size - len(input_list)
            # input_list += [PAD_INDEX] * pad_counts
